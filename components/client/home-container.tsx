  ...
  return (
   <>
      ...
      <Grid xs={12} sm={10}>
        <Collapse.Group css={{
          ...
          {(state?.sheet && state?.sheet.acts) ? state?.sheet.acts.map( (item, index ) => (
            <>
              <Collapse title={<CollapseTitle title={item.act.name} remove={() => removeAct(item.act.id)} open={() => {
                setOpen(true)
                setActId(item.act.id)
              }}/>} subtitle={`Total beats: ${item.beats?.length}   ID: ${item.act.id}`} key={`${item.act.id}`}>
                <Row gap={2} css={{
                  overflow: 'scroll',
                  margin: 0
                }}>
                  {item.beats ? item.beats.map((beat, i) => (
                    <Grid xs={12} sm={4} key={beat.id} css={{
                      pl:0
                    }}>
                      <BeatCard beat={beat} actId={item.act.id}/>
                    </Grid>
                  )): null}
                </Row>
              </Collapse>
    ...
    <ModalContainer isOpen={isOpen} closeHandler={() => setOpen(false)} title='New Beat'>
      <NewModal actId={actId} close={() => setOpen(false)}/>
    </ModalContainer>
  </>
  )
}
...
const CollapseTitle = ({ title, open, remove }: CollapseTitleProps) => {
  return (
    <Grid.Container gap={1}>
       ...
       <Grid xs={4} sm={1} justify='center' alignItems='center'>
        <Button size='xs' onPress={() => open()}>
          Create Beat
        </Button>
       </Grid>
       <Grid xs={4} sm={1} justify='center' alignItems='center'>
        <Button color='error' size='xs' onPress={() => remove()}>
          Delete Act
        </Button>
       </Grid>
    </Grid.Container>
